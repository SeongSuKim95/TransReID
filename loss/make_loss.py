...
def make_loss(cfg, num_classes):    # make loss는 class가 아닌 definition
    ...
    if "triplet" in sampler :
        if loss_type == "triplet":
            ...
            if cfg.MODEL.NO_MARGIN:
                triplet = TripletAttentionLoss_ss_pos_6(loss_ratio,patch_ratio,num_instance,max_epoch,rel_pos,comb,replacement,comb_idx,jsd,head_wise,head_num)
                print("using soft triplet_ss_pos_6 attention loss for training with loss ratio : {} ,patch ratio : {}".format(loss_ratio,patch_ratio))
            else:
                triplet = TripletAttentionLoss_ss_pos_6(loss_ratio,patch_ratio,num_instance,max_epoch,rel_pos,comb,replacement,comb_idx,jsd,head_wise,head_num,cfg.SOLVER.MARGIN)  # triplet loss
