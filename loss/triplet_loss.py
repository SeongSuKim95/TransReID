...
class TripletAttentionLoss_ss_pos_6(object):
    ...
    def __call__(
        ...
        if self.rel_cls:
            ...
            # rel_pos_bias_p = rel_pos_bias[:,:,1:]
            # rel_pos_bias_c = rel_pos_bias[:,:,0]
            abs_pos_p = abs_pos[1:]
            # abs_pos_c = abs_pos[0]
            # anc_vec_c = []
        else : 
        ...
        for i in range(B):
            if self.rel_pos :
                if self.rel_cls :
                    rel_val_anc_p = torch.stack([rel_pos_bias_p[:,x[0],x[1]] for x in anc_comb[i]])
                    abs_val_anc_p = torch.stack([abs_pos_p[x[0]] @ abs_pos_p[x[1]] for x in anc_comb[i]])
                    ...
                    # rel_val_anc_c = rel_val_anc_c.softmax(0).view(-1)
                    ...
                    abs_val_anc = abs_val_anc.unsqueeze(1).expand(rel_val_anc.size(0),rel_val_anc.size(1))
                    anc_vec.append(rel_val_anc+abs_val_anc)
                    # anc_rel.append(rel_val_anc)
                    #anc_abs.append(abs_val_anc)
            else :
                abs_val_anc = torch.stack([abs_pos[x[0]] @ abs_pos[x[1]] for x in anc_comb[i]])
                anc_vec.append(abs_val_anc)
        ...
        anc_vec = (torch.cat(anc_vec).reshape(B,anc_comb.size(1),-1).transpose(-2,-1).softmax(-1)).reshape(B,-1)
        # anc_rel = (torch.cat(anc_rel).reshape(B,anc_comb.size(1),-1).transpose(-2,-1).softmax(-1)).reshape(B,-1)
        # anc_abs = (torch.cat(anc_abs).reshape(B,anc_comb.size(1)).softmax(-1))
        
        # CLS position vector 
        # if self.rel_cls:
        #         anc_vec_c = torch.cat(anc_vec_c).reshape(B,-1)
        ...
        if self.comb :
            if self.HEAD_WISE:
                ...
                if self.REPLACEMENT:
                    ...
                    # anc_rel = anc_rel.reshape(B,-1,int((self.HEAD_NUM * self.comb_idx * (self.comb_idx + 1))/2))
                else :
                    anc_vec = anc_vec.reshape(B,-1,int((self.HEAD_NUM * self.comb_idx * (self.comb_idx - 1))/2))
        else :
            if self.HEAD_WISE:
        ...
        # pos_rel,pos_abs = anc_rel[ind_pos_cls],anc_abs[ind_pos_cls]
        # pos_rel = anc_rel[ind_pos_cls]
        #################################################################################
        # cat_pos = torch.cat((ind_anc,ind_pos),dim=-1)
        ...
        if self.margin is not None:
            ...
            if self.JSD : 
                if self.rel_cls:
                    if self.HEAD_WISE : 
                        ...
                        position_loss_a = position_loss_p 
                else :
                    if self.HEAD_WISE : 
                        position_loss_a = self.JSD_loss(anc_vec,pos_vec)
                        ...
                        #position_loss = position_loss_abs + position_loss_rel
                        if self.MEAN_POS : 
                            position_loss_a = self.KLD_mean(anc_vec,anc_vec)
                        else :
                            position_loss_a = self.JSD_loss(anc_vec,pos_vec)

            else :
                position_loss_p = self.KLD_loss(anc_vec.log(),pos_vec.log())
            #loss_gap = self.ranking_loss(dist_an - dist_ap, y)
            loss_cls = self.ranking_loss(dist_an_cls - dist_ap_cls, y)
            # loss_cls_detach = self.ranking_loss(dist_an_cls - dist_ap_cls.detach(),y)
            #loss_cls_mean = self.ranking_loss(dist_an_mean_cls - dist_ap_cls.detach(),y)
            ...
            if torch.isnan(loss) or torch.isinf(loss) :
                wandb.finish()
        if self.rel_cls:
            return loss, loss_cls, position_loss_p, p_ratio, dist_ap_cls, dist_an_cls
        else :
            return loss, loss_cls, position_loss_a, p_ratio, dist_ap_cls, dist_an_cls 
